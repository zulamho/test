...
import {StyleSheet, View, Text, TouchableOpacity, ScrollView, SafeAreaView, TextInput, Modal, Switch, Dimensions, KeyboardAvoidingView, Alert, Linking} from 'react-native';
...
const GroupPermisionModal = ({isVisible, setIsVisible}) => {
	const name = 'Эдуард';
	const lastname = '123';

	const [isDeleteMembersOn, setIsDeleteMembersOn] = useState('');
	const [isMemberAddOn, setIsMemberAddOn] = useState('');
	const [isExchangeGroupOn, setIsExchangeGroupOn] = useState('');
		...
		<Modal animationType="fade" transparent={false} statusBarTranslucent={false} visible={isVisible} onRequestClose={() => {}}>
			<View
				...
				<ScrollView
					...
					<Switcher title={'Добавление участников'} icon={'user-plus'} isEnabled={isMemberAddOn} toggleIsEnabled={setIsMemberAddOn}></Switcher>
					<Switcher title={'Изменения профиля группы'} icon={'edit'} isEnabled={isExchangeGroupOn} toggleIsEnabled={setIsExchangeGroupOn}></Switcher>
					<Switcher title={'Удаление участников'} icon={'user-minus'} isEnabled={isDeleteMembersOn} toggleIsEnabled={setIsDeleteMembersOn}></Switcher>
					<View
						contentContainerStyle={{alignItems: 'center'}}
						style={{
							backgroundColor: '#E6E6E6',
					...
					<View style={{height: 290}}>
						<ScrollView>{/* <AddMember user={user}></AddMember> */}</ScrollView>
					</View>
					<Divider />
					<Divider />
					<Divider />
			...
			<ContactsListModal isVisible={ContactsListModalVisible} setIsVisible={setIsContactsListModalVisible}></ContactsListModal>
		</Modal>
	);
};
...
export default GroupPermisionModal;
